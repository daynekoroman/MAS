...
public class ServerPassengerBehaviour extends TickerBehaviour {
	...
	private void becomeChaufferAndInformAll() {
		...
		System.out.println(myAgent.getLocalName() + ": i'm chauffeur now!");
		...
		for(DriverDescription dd : potentialChauffeurs) {
			...
			System.out.println(myAgent.getLocalName() + 
				": inform about new chauffeur other potential chauffeur - " + dd.getAid().getLocalName());
		}
		for(DriverDescription dd : driverAgent.getPotentialPassengers()) {
			msg.addReceiver(dd.getAid());
			System.out.println(myAgent.getLocalName() + 
					": inform about new chauffeur other potential passenger - " + dd.getAid().getLocalName());
		}
    ...
    private void sendRequests() {
        ...
        if (potentialChauffeurs.size() == 0) {
        	...
        	System.out.println(driverAgent.getLocalName() + 
        			": i'm chauffeur because there is no one good driver");
        	return;
        }
        ...
        for (DriverDescription driver : potentialChauffeurs) {
            ...
            System.out.println(driverAgent.getLocalName() + " I send a message to " + 
    ...
    private void receiveRequestFromPotentialChauffeurs() {
        ...
        while (true) {
        	...
        	if (newCost > passengerCoefficientLimit * costsForChauffers.get(msg.getSender().getName())) {
        		...
        		System.out.println(driverAgent.getLocalName() + ": refuse because this guy - " + 
        		...
        		if (potentialChauffeurs.size() == 0) {
        			...
        			becomeChaufferAndInformAll();
                	System.out.println(driverAgent.getLocalName() + 
        			": i'm chauffeur because other chauffeurs are too expensive or rejected my proposal");
                	return;
        		}
    ...
    private void receiveRequestFromPotentialPassengers() {
        ...
        while (true) {
            ...
            if (flag) {
	            System.out.println(myAgent.getLocalName() + ": i(passenger) add new potential passenger - " +
	            		msg.getSender().getLocalName());
	            ...
	            System.out.println(driverAgent.getLocalName() + ": i got msg CFP from potential passenger " + 
	            		msg.getSender().getLocalName());
            }
        }
        ...
        if (!driverAgent.isChauffeur()) {
        	if (!waitForConfirm && shouldBeChauffeur()) {
        		becomeChaufferAndInformAll();
	        	System.out.println(driverAgent.getLocalName() + ": i'm chauffeur because it's more profitable");
	        }
        	else {
        		for(ACLMessage message : messages) {
        			...
        			driverAgent.send(newMsg);
        			System.out.println(driverAgent.getLocalName() + ": need more money, sended to " + 
    ...
    private void handleInformAboutChauffeur() {
    	...
    	while (true) {
        	...
        	driverAgent.removePotentialPassenger(msg.getSender());
        	System.out.println(myAgent.getLocalName() + ": i've got message about new chauffeur - " + 
        			msg.getSender().getLocalName());
    	}
    }
    ...
    private void handleAcceptFromPotentialChauffeur() {
    	...
    	while (true) {
        	...
        	System.out.println(myAgent.getLocalName() + ": i've handled accept proposal from chauffeur - " + 
        			msg.getSender().getLocalName());
    		waitForConfirm = true;
    		ACLMessage newMsg = msg.createReply();
    	...
    	while (true) {
    		...
    		for(DriverDescription dd : driverAgent.getDrivers()) {
    			if (dd.getAid() != msg.getSender() && !dd.getAid().getLocalName().equals(myAgent.getAID().getLocalName())) {
    				...
    				System.out.println(driverAgent.getLocalName() + ": notificate that i'm gone like passenger " + 
    						dd.getAid().getLocalName());
    			}
    		}
    		
    		driverAgent.send(newMsg);
    		System.out.println(driverAgent.getLocalName() + ": i'm gone like passenger(with " + 
    				msg.getSender().getLocalName() + " )");
    		myAgent.doDelete();
    		break;
    	}
    ...
    private void handleInformAboutLeaving() {
    	...
    	while (true) {
        	...
        	System.out.println(myAgent.getLocalName() + ": i've(passenger) got msg about leaving " + 
        			msg.getSender().getLocalName());
        	...
        	if (!driverAgent.isChauffeur() && potentialChauffeurs.isEmpty()) {
        		...
        		becomeChaufferAndInformAll();
        		System.out.println(driverAgent.getLocalName() + 
            			": i'm chauffeur because all guys leaving");
        		return;
        	}
